...
// Constructor
PeanoHilbert::PeanoHilbert(int dimensions, int max_depth) 
    : dimensions(dimensions), max_depth(max_depth) {
    if (dimensions != 3) {
        throw std::invalid_argument("Currently only 3D is supported");
    }
}

// Generate Peano-Hilbert key for a single position (32-bit)
uint32_t PeanoHilbert::generate_key(uint32_t x_in, uint32_t y_in, uint32_t z_in, int depth) {
    if (depth == -1) {
        depth = max_depth;
    }
    
    // Calculate shift to extract the 'depth' most significant bits
    int shift_amount = 32 - depth; // assuming 32-bit input
    ...
    for (int i = 0; i < depth; i++) {
        ...
        key = (key << 3) | static_cast<uint32_t>(subpix3[rotation][pix]);
...
uint64_t PeanoHilbert::generate_key_64(uint32_t x_in, uint32_t y_in, uint32_t z_in, int depth) {
    if (depth == -1) {
        depth = max_depth;
    }
...
std::vector<uint32_t> PeanoHilbert::generate_keys(const PositionVector& positions, int depth) {
...
std::vector<uint64_t> PeanoHilbert::generate_keys_64(const PositionVector& positions, int depth) {
    std::vector<uint64_t> keys;
    keys.reserve(positions.size());
    
    for (const auto& pos : positions) {
        keys.push_back(generate_key_64(pos[0], pos[1], pos[2], depth));
    }
